...
| Module | Role |
|--------|------|
| `haluguard/hccs.py` | `HallucinationType` enum + frozen CodeBERT helpers + legacy `HCCSScorer` MLP |
| `haluguard/models.py` | **All 7 scorer architectures** (DualEncoder, DualEncoderDeep, ListwiseMLP, PairwiseMLP, InteractionMLP, BilinearScorer, EnsembleScorer) + `MODEL_REGISTRY` |
...
| `haluguard/training.py` | Model-agnostic listwise training, curriculum learning, focal loss, SWA support |
| `haluguard/retrieval_benchmark.py` | Bucketing + retrieval metrics helpers (easy/hard candidate sets) |

## Scorer architectures (haluguard/models.py)

| Name | Params | Key technique | Loss |
|------|--------|---------------|------|
| `DualEncoder` | ~200K | Cosine sim + learned temperature | Listwise CE |
| `DualEncoderDeep` | ~800K | 2-layer projection | Listwise CE |
| `ListwiseMLP` | ~400K | concat[q,c] + LayerNorm + GELU | Listwise CE |
| `PairwiseMLP` | ~400K | concat[q,c] + Sigmoid | InfoNCE |
| `InteractionMLP` | ~1.1M | ESIM features + spectral norm | Listwise CE |
| `BilinearScorer` | ~98K | Factorized bilinear (lowest capacity) | Listwise CE |
| `EnsembleScorer` | tiny gate | Learned mixture over top-3 models | Listwise CE |

## Import dependency order (no circular imports)

```
hccs  ←  models  ←  training
...
- `data/embeddings/` — pre-computed `.pt` CodeBERT/UniXcoder embeddings
